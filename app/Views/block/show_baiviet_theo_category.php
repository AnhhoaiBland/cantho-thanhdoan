...
// Function to get posts by category name
function get_posts_by_category($category_name) {
    // Assuming you have a database connection established
    global $db;

    // Prepare the SQL query to get posts by category name
    $query = $db->prepare("SELECT * FROM posts WHERE category_name = :category_name");
...
// Example usage
$category_name = $chuyenMuc[0]['tenChuyenMuc'];
$ds_baiDang = get_posts_by_category($category_name);
?>

<div class="container-fluid container-fluid-card bg-body" style="min-height: 700px;">

    <div>
        <h5 class="category_title"><?= $category_name ?></h5>
    </div>
    ...
    <div class="container-card">
        ...
        <div class="card">
            <a
                href="<?= $baidang['urlBaiDang'] ? base_url('/bv/' . $baidang['urlBaiDang']) : base_url('/bv/' . $baidang['maBaiDang']) ?>">
